...
<head>
    ...
    <meta property="og:site_name" content="http://localhost/flowershopdalat/public/" />
    ...
    <link rel="shortcut icon" type="image/png" href="client_asset/assets/dest/img/favicon.png"/>
    <!-- Css Styles -->
...
<div class="modal fade bd-example-modal-lg" id="QuickView" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        function add_wistlist(clicked_id) {
            ...
            if(matches.length){
                swal("FlowerShop-DaLat", "Không thể yêu thích sản phẩm thêm lần nữa", "warning");
            }else{
                swal("FlowerShop-DaLat", "Cảm ơn bạn đã yêu thích sản phẩm", "success");
        ...
        function delete_withlist() {
            if(localStorage.getItem('data')!=null){
                ...
                swal("FlowerShop-DaLat", "Bạn đã tất cả sản phẩm yêu thích", "success").then(ok=>{window.location.reload()});
            }
            else
            {
                swal("FlowerShop-DaLat", "Không tồn tại sản phẩm yêu thích!!!", "warning");
            }
            localStorage.setItem('data',JSON.stringify(old_data));
        };
    </script>
    ...
    <script>
        ...
        Push.create("Chào bạn đến với FlowerShopDaLat!", {
